...
export const foundryRouter = router({
  ...
  session: router({
    ...
    command: protectedProcedure.input(z.object({ taskId: z.number().int().positive(), type: z.enum(["refresh", "approve_plan", "send_message", "request_delete", "set_local_hold", "release_local_hold", "reconcile", "export_dossier"]), reason: z.string().max(1500).optional(), message: z.string().max(3000).optional(), confirmation: z.string().max(180).optional() })).mutation(async ({ ctx, input }) => {
      ...
      const payload = { reason: input.reason ?? "", message: input.message ?? "", confirmation: input.confirmation ?? "" };
      const existingControl = (await db.select().from(sessionControls).where(eq(sessionControls.idempotencyKey, sessionControlKey(record.task.id, input.type as SessionControlType, payload))).limit(1))[0];
      if (existingControl) return { reused: true, control: existingControl };
      ...
      const begun = await beginSessionControl({ taskId: record.task.id, sessionName: record.task.julesSessionName, type: input.type as SessionControlType, userId: user, payload, reason: input.reason, snapshot });
