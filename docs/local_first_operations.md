...
| Credential vault | AES-256-GCM ciphertext using a random key stored in the OS credential store; `scrypt` passphrase mode is an explicit recovery fallback. | Native secure storage is preferred. If it is unavailable, set `FOUNDRY_VAULT_MODE=passphrase` and the same passphrase before launch. |
...
| Process safety | Owner-only data-directory lock, startup storage preflight, and orderly database close. | A second application launch must use the existing instance rather than share the SQLite ledger. |
| Operations | Local operations workspace with redacted diagnostics, backup inventory, staged restore, and retention preferences. | The screen is informational and recovery-focused; it never exposes secrets or silently overwrites live data. |

## First run

Before first use, Foundry attempts to create a random vault key in the current user’s operating-system credential store. If that facility is unavailable, set `FOUNDRY_VAULT_MODE=passphrase` and `FOUNDRY_VAULT_PASSPHRASE` in the environment that launches Foundry. A passphrase manager or operating-system secret launcher is appropriate; do not put this value in Git, a shell history, issue tracker, or support bundle.
...
Credential values remain write-only. A saved provider key can be tested, rotated, or deleted, but never read back. A local SQLite backup contains encrypted credential blobs only. If a device is replaced, the original operating-system credential entry and recovery passphrase are unavailable, or the vault is intentionally reset, re-enter provider credentials through the dashboard.
...
Create backups through the Credential vault or Local operations control. The service uses SQLite `VACUUM INTO` so the result is consistent even when the working database has a WAL file. The service validates a backup with `PRAGMA integrity_check` before treating it as restorable. Local operations can retain a selected number of snapshots and prune only older backup files.
...
| Credential vault reports locked | Confirm the current OS user has access to its credential store. If secure storage is unavailable, start in explicit passphrase mode with the same `FOUNDRY_VAULT_PASSPHRASE` used to encrypt local credentials, or re-enter credentials under a new vault key. |
| Foundry reports another local instance | Close or focus the existing instance. Do not delete `foundry.instance.lock` unless you have confirmed the recorded process is no longer running. |
...
```bash
FOUNDRY_VAULT_MODE=passphrase FOUNDRY_VAULT_PASSPHRASE="test-only-value" pnpm check
FOUNDRY_VAULT_MODE=passphrase FOUNDRY_VAULT_PASSPHRASE="test-only-value" pnpm test
FOUNDRY_VAULT_MODE=passphrase FOUNDRY_VAULT_PASSPHRASE="test-only-value" pnpm release:verify
```
