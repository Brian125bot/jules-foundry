...
function InitiativeQualityGate({ initiativeId }: { initiativeId: number }) {
  ...
  const contractBody = contract ? JSON.parse(contract.contractJson) as { outcome?: string; operatorQuestions?: string[] } : null;
...
export default function Initiatives() {
  ...
  const create = trpc.foundry.initiatives.create.useMutation({
    onSuccess: async result => {
      ...
      finally { utils.foundry.initiatives.list.invalidate(); setComposerOpen(false); setTitle(""); setPrompt(""); }
    },
  });
  ...
  const submitCreate = (event: FormEvent) => { event.preventDefault(); create.mutate({ title, prompt, repository, branch, baseSha: baseSha || undefined, budgetCents: Number(budget) || 500 }); };
