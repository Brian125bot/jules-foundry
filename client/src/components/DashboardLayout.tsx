...
import { Activity, Bot, Boxes, CheckCircle2, ChevronRight, CircleHelp, GitPullRequest, KeyRound, LayoutDashboard, LogOut, PanelLeft, Radar, ShieldCheck, Workflow } from "lucide-react";
  ...
  return <>
    <div ref={sidebarRef} className="relative">
      <Sidebar collapsible="icon" className="border-r border-white/10 bg-[#0b1020] text-slate-200" disableTransition={isResizing}>
        ...
        <SidebarFooter className="space-y-2 p-3"><Button variant="ghost" onClick={() => setAboutOpen(true)} className="h-10 w-full justify-start rounded-xl px-3 text-slate-300 hover:bg-white/7 hover:text-white" aria-label="About and how to use Jules Foundry"><CircleHelp className="h-4 w-4 shrink-0" /><span className="ml-2 group-data-[collapsible=icon]:hidden">About &amp; how to use</span></Button><DropdownMenu><DropdownMenuTrigger asChild><button className="flex w-full items-center gap-3 rounded-xl p-2 text-left transition hover:bg-white/6"><Avatar className="h-8 w-8 border border-white/10"><AvatarFallback className="bg-slate-800 text-xs text-cyan-200">{user?.name?.charAt(0).toUpperCase() || "U"}</AvatarFallback></Avatar><div className="min-w-0 group-data-[collapsible=icon]:hidden"><p className="truncate text-xs font-medium text-white">{user?.name || "Operator"}</p><p className="mt-0.5 truncate text-[10px] text-slate-500">Private workspace</p></div></button></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem onClick={logout} className="text-destructive"><LogOut className="mr-2 h-4 w-4" />Sign out</DropdownMenuItem></DropdownMenuContent></DropdownMenu></SidebarFooter>
      </Sidebar>
