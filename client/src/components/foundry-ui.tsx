...
export function ConsoleHeader({ eyebrow, title, description, action }: { eyebrow: string; title: string; description: string; action?: ReactNode }) {
  return <header className="border-b border-slate-200 bg-white px-5 py-6 sm:px-8 lg:px-10"><div className="mx-auto flex max-w-[1600px] flex-col items-start justify-between gap-4 sm:flex-row sm:gap-5"><div><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-cyan-700">{eyebrow}</p><h1 className="mt-2 text-2xl font-semibold tracking-tight text-slate-950 sm:text-3xl">{title}</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">{description}</p></div>{action}</div></header>;
}
